...
ROBOT_POSE_TOPIC = '/robot4/amcl_pose'
...
class Amr2Mission(Node):
    ...
    def __init__(self):
        ...
        # OCR 결과 수신.
        self.create_subscription(
            Bool, OCR_RESULT, self._ocr_callback, 10)
        # 웹캠 JSON map 좌표 수신.
        self.create_subscription(
        ...
        self.get_logger().info('AMR2 simple mission node ready.')
    ...
    def _amr1_json_to_mission(self, msg: String):
        ...
        try:
            ...
            return {
                'id': goal_id,
                'zone': self._zone_to_number(data.get('zone')),
                'x': float(point['x']),
                'y': float(point['y']),
                'yaw': 0.0,
            }
    ...
    def _webcam_target_to_mission(self, target):
        ...
        robot_x = self.latest_robot_pose.pose.position.x
        robot_y = self.latest_robot_pose.pose.position.y
        dx = car_x - robot_x
        dy = car_y - robot_y
        ...
        if distance < 1e-6:
            self.get_logger().warning(
                '로봇과 차량 좌표가 거의 같아 차량 좌표를 그대로 사용합니다.')
            return {
                'id': str(target['id']),
                'zone': target['zone'],
                'x': car_x,
        ...
        offset = min(WEBCAM_TARGET_OFFSET_M, distance)
        goal_x = car_x - dx / distance * offset
        goal_y = car_y - dy / distance * offset
    ...
    def json_to_dic(self, msg: String):
        ...
        try:
            ...
            for index, det in enumerate(objects):
                zone = self._zone_to_number(det.get('zone'))
                if zone not in (3, 4):
                    continue
    ...
    def _zone_to_number(self, zone):
        """zone3, zone4 같은 값을 OCR에 보낼 숫자 zone으로 변환한다."""
        if zone is None:
            return None
    ...
    def _robot_pose_callback(self, msg: PoseWithCovarianceStamped):
        """AMCL이 알려주는 현재 로봇 위치를 저장한다."""
        # AMCL 메시지는 PoseWithCovarianceStamped이고,
        # Nav2 goal에는 PoseStamped가 필요하므로 필요한 pose만 옮겨 담는다.
        pose = PoseStamped()
    ...
    def _start_ocr_wait(self):
        ...
        if img_msg is None:
            self.get_logger().warning(
                '차량 이미지 생성 실패. OCR 흐름 테스트용 빈 Image를 발행합니다.')
            img_msg = Image()
        ...
        target_msg.data = json.dumps({
            ...
            'zone': self.current_mission.get('zone'),
        })
        self.id_to_ocr.publish(target_msg)
        self.get_logger().info('목표 도착. OCR 결과 대기')

    def _make_target_plate_image(self, mission) -> Optional[Image]:
        """현재 mission에 대해 OCR에 보낼 차량 이미지를 만든다.
        해벽님 차량/번호판 탐지 코드가 들어갈 자리
        sensor_msgs.msg.Image 형태로 반환
        """
        if mission is None:
            return None
        ...
        # TODO(해벽님 탐지 코드):
        # - event_id, zone, goal_x, goal_y를 필요하면 사용한다.
        # - 현재 카메라 이미지에서 대상 차량/번호판 이미지를 찾는다.
        # - 결과를 sensor_msgs.msg.Image로 변환해서 반환한다.
        # - 탐지 실패 시 None을 반환하면 빈 Image로 대체 발행된다.
        _ = event_id, zone, goal_x, goal_y
        return Image()

    def _ocr_callback(self, msg: Bool):
        ...
        self.get_logger().info(f'OCR 결과: {msg.data}')
