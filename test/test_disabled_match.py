"""
test_disabled_match.py — 장애인 주차구역 OCR 테스트 (ROS2 없이 단독 실행)
...
테스트 흐름:
  OCR → 번호판 텍스트 추출
      → GET /api/disabled/<plate>/ → 장애인 차량 등록 여부 확인
      → True  (등록된 장애인 차량, 정상주차) → 이벤트 삭제
      → False (미등록, 불법주차)             → WARNING_ISSUED

사용법:
  python3 test_disabled_match.py <이미지경로> <db_id>

  예시:
    python3 test_disabled_match.py /home/rokey/Downloads/plate.jpg 1
"""
...
# ── 장애인 차량 등록 여부 조회 ────────────────────────────────
def check_disabled(plate_number):
    resp = requests.get(f"{SERVER}/api/disabled/{plate_number}/", timeout=5)
    if resp.status_code == 200:
        is_disabled = resp.json().get("is_disabled", False)
        print(f"[SERVER] 장애인 차량 등록 여부: {is_disabled}")
        return is_disabled
    print(f"[ERROR] 조회 실패: {resp.status_code} {resp.text}")
    return None
...
def main():
    if len(sys.argv) != 3:
        print("사용법: python3 test_disabled_match.py <이미지경로> <db_id>")
        print("예시:   python3 test_disabled_match.py /home/rokey/Downloads/plate.jpg 1")
        sys.exit(1)
    ...
    # 2. 장애인 차량 등록 여부 확인
    is_disabled = check_disabled(plate_text)
    if is_disabled is None:
        print("[FAIL] 서버 조회 실패 — 종료")
        sys.exit(1)

    # 3. 결과 처리
    if is_disabled:
        print(f"\n[결과] 등록된 장애인 차량 → 정상주차 → 이벤트 삭제")
        send_normal(db_id)
    else:
        print(f"\n[결과] 미등록 차량 → 불법주차 → WARNING_ISSUED")
